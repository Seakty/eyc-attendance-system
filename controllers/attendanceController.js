...
async function scanAttendance(req, res) {
  ...
  let parsedQR;
  try {
    // Attempt to read the JSON generated by your settings page
    parsedQR = JSON.parse(qrToken);
  } catch (error) {
    // If it's not valid JSON (like a random text QR), kick it out!
    return res
      .status(400)
      .json({ status: "error", message: "Invalid QR Code format." });
  }

  // Verify this is actually an EYC system QR code
  if (parsedQR.type !== "eyc_entrance_qr") {
    return res
      .status(403)
      .json({ status: "error", message: "Unrecognized QR Code." });
  }
  // ==========================================
