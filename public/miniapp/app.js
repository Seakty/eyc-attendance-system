...
async function sendLocationToServer(lat, lng) {
  const locationRes = await fetch("/api/attendance/check-location", {
    method: "POST",
    headers: {
  ...
  if (!locationRes.ok) {
    throw new Error(locationData.message || "Location check failed.");
  }

  if (!locationData.isInside) {
    warningBox.classList.remove("hidden");
    distanceInfo.textContent = `You are ${locationData.distanceMeters}m away (allowed: ${locationData.allowedRadiusMeters}m).`;
    return;
  }
  ...
  statusMessage.textContent = checkInData.message || "Check-in successful.";
...
async function startQrScanner() {
  if (!window.Html5Qrcode || !qrScanner || qrScannerActive) return;
  ...
  try {
    ...
    qrReader = new Html5Qrcode("qrScanner");
    qrScannerActive = true;
    statusMessage.textContent = "Camera ready. Scan the QR code.";
    ...
    await qrReader.start(
      ...
      async (decodedText) => {
        try {
          ...
          const timestampText = new Date(data.timestamp || Date.now()).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
          });

          statusMessage.textContent = `${data.message || "Attendance recorded."} at ${timestampText}`;
          scannerBox.classList.remove("hidden");
          stopQrScanner();
        } catch (error) {
    ...
    statusMessage.textContent = "Unable to open the camera.";
